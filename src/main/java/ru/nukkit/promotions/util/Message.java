...
public enum Message {
    ...
    HLP_TITLE("%1% | Help"),
    DBPROMOTE_DESC ("/dbpromote <player> <group> [<timeStart>] <timeEnd> - test DbPromotion module"),
    INFORM_PROMOTE("Congratulations! You added to group %1%"),
    INFORM_DEMOTE("You was excluded from group %1%"),

    LOG_PROMOTE("[%3%] Player %1% added to group %2%"),
    LOG_DEMOTE("[%3%] Player %1% removed from the group %2%"),
    DBPROMOTE_ADDED("New promotion record for %1% created");
    ...
    public static void init(PluginBase plg) {
        plugin = plg;
        language = MultiPromotions.getCfg().language;
        ...
        debugMode = MultiPromotions.getCfg().debugMode;
        saveLanguage = MultiPromotions.getCfg().saveLanguage;
     ...
     * @param exception
     */
    public static void debugException(String title, Exception exception){
        if (!debugMode) return;
